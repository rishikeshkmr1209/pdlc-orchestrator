...
- NEVER auto-apply changes — always present the proposed diff and wait for user confirmation in plain text chat.
...
2. **Handle no files found (early exit):**
   ...
   - Ask the user in plain text: "No context files found. Want me to generate a starter CLAUDE.md for your <ecosystem> project? Reply **yes** or **no**."
   - **STOP your message here.** Wait for the user to reply in the next turn.
...
4. **Cross-tool duplication analysis (REQ-013, REQ-014):**
   ...
   - Ask the user in plain text: "Which AI coding tools do you use with this repo? (select all that apply): **(a)** Claude Code, **(b)** GitHub Copilot, **(c)** Cursor, **(d)** Windsurf, **(e)** Other — please specify."
   - **STOP your message here.** Wait for the user to reply in the next turn.
...
After presenting all outputs, ask the user in plain text:

> The proposed diff has **N changes** (R removals, V revisions, A additions). How would you like to proceed?
> **(a)** Apply all changes
> **(b)** Apply removals only
> **(c)** Skip — report only

**STOP your message here.** Wait for the user to reply in the next turn.

Handle responses (in your NEXT message):
- **Apply all (a):** Use Edit tool to apply each change. Report summary of applied changes.
- **Apply removals only (b):** Use Edit tool to remove REMOVE-verdict lines only. Report what was removed.
- **Skip (c):** Exit without modifying files.
...
Ask the user in plain text: "Generate reference files for <tool1>, <tool2> pointing to <primary_file> as source of truth? Reply **yes** or **no**."

**STOP your message here.** Wait for the user to reply in the next turn.

If approved (in your NEXT message): Write reference files using Write tool. Each reference file contains a pointer to the primary file and any tool-specific overrides that were unique to that file.
